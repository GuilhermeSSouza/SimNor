...
_lr_productions = [
  ...
  ('prog -> funcs','prog',1,'p_prog','lexerAndParser.py',172),
  ('prog -> externs funcs','prog',2,'p_prog','lexerAndParser.py',173),
  ('externs -> extern','externs',1,'p_externs','lexerAndParser.py',180),
  ('externs -> extern externs','externs',2,'p_externs','lexerAndParser.py',181),
  ('funcs -> func','funcs',1,'p_funcs','lexerAndParser.py',189),
  ('funcs -> func funcs','funcs',2,'p_funcs','lexerAndParser.py',190),
  ('extern -> EXTERN TYPE GLOBID LParen RParen Semicolon','extern',6,'p_extern','lexerAndParser.py',199),
  ('extern -> EXTERN TYPE GLOBID LParen tdecls RParen Semicolon','extern',7,'p_externWithTypes','lexerAndParser.py',203),
  ('func -> DEF TYPE GLOBID LParen RParen blk','func',6,'p_func','lexerAndParser.py',208),
  ('func -> DEF TYPE GLOBID LParen vdecls RParen blk','func',7,'p_funcWithParams','lexerAndParser.py',212),
  ('blk -> LBracket stmts RBracket','blk',3,'p_blk','lexerAndParser.py',217),
  ('blk -> LBracket RBracket','blk',2,'p_blkEmpty','lexerAndParser.py',221),
  ('stmts -> stmt','stmts',1,'p_statements','lexerAndParser.py',226),
  ('stmts -> stmt stmts','stmts',2,'p_statements','lexerAndParser.py',227),
  ('stmt -> blk','stmt',1,'p_blkStmt','lexerAndParser.py',236),
  ('stmt -> Return Semicolon','stmt',2,'p_return','lexerAndParser.py',240),
  ('stmt -> Return exp Semicolon','stmt',3,'p_return','lexerAndParser.py',241),
  ('stmt -> vdecl Equal exp Semicolon','stmt',4,'p_vdeclStmt','lexerAndParser.py',249),
  ('stmt -> exp Semicolon','stmt',2,'p_expSemi','lexerAndParser.py',261),
  ('stmt -> While LParen exp RParen stmt','stmt',5,'p_while','lexerAndParser.py',265),
  ('stmt -> If LParen exp RParen stmt','stmt',5,'p_if','lexerAndParser.py',269),
  ('stmt -> If LParen exp RParen stmt Else stmt','stmt',7,'p_if','lexerAndParser.py',270),
  ('stmt -> Print exp Semicolon','stmt',3,'p_print','lexerAndParser.py',277),
  ('stmt -> Array LSquare lit RSquare PLUS lit Semicolon','stmt',7,'p_arrayOperator','lexerAndParser.py',281),
  ('stmt -> Array LSquare GLOBID RSquare PLUS lit Semicolon','stmt',7,'p_arrayOperator','lexerAndParser.py',282),
  ('stmt -> Array LSquare lit RSquare Minus lit Semicolon','stmt',7,'p_arrayOperator','lexerAndParser.py',283),
  ('stmt -> Array LSquare GLOBID RSquare Minus lit Semicolon','stmt',7,'p_arrayOperator','lexerAndParser.py',284),
  ('stmt -> GLOBID Equal Vector Semicolon','stmt',4,'p_arrayGLOBID','lexerAndParser.py',289),
  ('Vector -> Array LSquare RSquare','Vector',3,'p_vector','lexerAndParser.py',294),
  ('Vector -> Array LSquare lit RSquare','Vector',4,'p_vector','lexerAndParser.py',295),
  ('Vector -> Array LSquare GLOBID RSquare','Vector',4,'p_vector','lexerAndParser.py',296),
  ('exps -> exp','exps',1,'p_exps','lexerAndParser.py',306),
  ('exps -> exp COMMA exps','exps',3,'p_exps','lexerAndParser.py',307),
  ('exp -> LParen exp RParen','exp',3,'p_expParen','lexerAndParser.py',323),
  ('exp -> lit','exp',1,'p_exp','lexerAndParser.py',327),
  ('exp -> binop','exp',1,'p_expBinOpUop','lexerAndParser.py',338),
  ('exp -> GLOBID','exp',1,'p_var','lexerAndParser.py',342),
  ('exp -> GLOBID expWrapper','exp',2,'p_expGlobid','lexerAndParser.py',346),
  ('expWrapper -> LParen RParen','expWrapper',2,'p_expWrapper','lexerAndParser.py',350),
  ('expWrapper -> LParen exps RParen','expWrapper',3,'p_expWrapper','lexerAndParser.py',351),
  ('binop -> exp Multiply exp','binop',3,'p_binop','lexerAndParser.py',359),
  ('binop -> exp PLUS exp','binop',3,'p_binop','lexerAndParser.py',360),
  ('binop -> exp Divide exp','binop',3,'p_binop','lexerAndParser.py',361),
  ('binop -> exp Minus exp','binop',3,'p_binop','lexerAndParser.py',362),
  ('binop -> GLOBID Equal exp','binop',3,'p_binop','lexerAndParser.py',363),
  ('binop -> exp Equality exp','binop',3,'p_binop','lexerAndParser.py',364),
  ('binop -> exp Different exp','binop',3,'p_binop','lexerAndParser.py',365),
  ('vdecls -> vdecl COMMA vdecls','vdecls',3,'p_vdecls','lexerAndParser.py',404),
  ('vdecls -> vdecl','vdecls',1,'p_vdecls','lexerAndParser.py',405),
  ('vdecl -> TYPE GLOBID','vdecl',2,'p_vdeclare','lexerAndParser.py',413),
  ('tdecls -> TYPE','tdecls',1,'p_tdecls','lexerAndParser.py',418),
  ('tdecls -> TYPE COMMA tdecls','tdecls',3,'p_tdecls','lexerAndParser.py',419),
  ('TYPE -> int','TYPE',1,'p_type','lexerAndParser.py',427),
  ('TYPE -> void','TYPE',1,'p_type','lexerAndParser.py',428),
  ('TYPE -> ref TYPE','TYPE',2,'p_refType','lexerAndParser.py',432),
  ('TYPE -> noalias ref TYPE','TYPE',3,'p_refTypeNoAlias','lexerAndParser.py',437),
]
